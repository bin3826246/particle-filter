...
inline bool write_best_particle_data(std::string fileName, std::vector<ground_truth>& best_particle){

    std::ofstream out_file_pos (fileName, std::ofstream::out);

    if(!out_file_pos){
        return false;
    }

    for (auto & bp : best_particle){
//        std::ostringstream oss_pos()

        out_file_pos << std::to_string(bp.x) + ' ';
        out_file_pos << std::to_string(bp.y) + ' ';
        out_file_pos << std::to_string(bp.theta) + '\n';
    }
    out_file_pos.close();
...
const static float STATIC_ONE_OVER_SQRT_2PI = 1.0/std::sqrt(2*M_PI);
